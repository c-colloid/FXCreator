...
>
> 実装は §4.2 のとおりで、テストも通っている。規則の方を実態に合わせるなら、
> 「2つの State が同じ bool で相互に遷移する対」を toggle と見なす規則の追加になるが、
> これは<b>2つの State を1ノードに畳む</b>ことを意味し、D5（ノード = AnimatorState の 1:1）と
> 正面からぶつかる。設計判断が要るため Phase 4 では触っていない。
